...
def _apply_masking(x0, cfg, fixed_t_step=None):
    """Mask tokens according to survival_prob.  Works on any device."""
    B, L = x0.shape
    device = x0.device
    if fixed_t_step is not None:
        t = torch.full((B,), fixed_t_step, device=device, dtype=torch.long)
    else:
        t = torch.randint(1, cfg["T"] + 1, (B,), device=device)
    a_t = cfg["survival_prob_tensor"](t).unsqueeze(1)
    token_mask = torch.rand(B, L, device=device) > a_t
    xt = x0.clone()
...
def make_batch(x0, cfg):
    """Apply random masking to clean tokens for training."""
    return _apply_masking(x0, cfg)


def make_eval_batch(x0, cfg, fixed_t_step=None, **kwargs):
    """Apply masking with optional fixed timestep for evaluation."""
